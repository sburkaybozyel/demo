...
function whatsappUrl(property) {
  const phone = property.contact?.whatsapp || "1234567890"; // Fallback for demo
  const text = `Hi ${property.name}, I'm interested in booking a stay.`;
...
function renderGallery(gallery) {
  ...
  return `
          ...
          <div class="g4"><p>Stop paying commissions. Connect directly with your guests.</p></div>
  ...
  return `
    <section class="section why-section" id="why">
      <div class="section-inner">
        <div class="why-top">
          <p class="kicker kicker--light">Why have your own site?</p>
          <h2 class="why-h2">Stop losing 20% on every booking.</h2>
          <p class="why-sub">Airbnb, Booking.com, and Expedia take 15–25% commission on every reservation. On a $10,000 booking, you lose $2,000 to fees. With your own site, that money stays in your pocket.</p>
        </div>
        <div class="why-grid">
          <div class="why-card why-bad">
            <div class="why-label why-label--bad">On Platforms</div>
            <ul>
              <li>15–25% commission fees per booking</li>
              <li>You don't own your guest data</li>
              <li>Your prices are shown next to competitors</li>
              <li>Your reputation is locked to the platform</li>
              <li>Policies can change without your consent</li>
            </ul>
          </div>
          <div class="why-card why-good">
            <div class="why-label why-label--good">On Your Own Site</div>
            <ul>
              <li>Zero commission — all profit is yours</li>
              <li>Direct payments via Stripe or PayPal</li>
              <li>Guests can contact you directly via WhatsApp</li>
              <li>Build your own brand in Google Search</li>
              <li>Own your customer data and loyalty</li>
            </ul>
          </div>
          <div class="why-card why-cta">
            <div class="why-label why-label--cta">Get Started</div>
            <ul>
              <li>Live in 7 days, fast setup</li>
              <li>Mobile-optimized, high-speed design</li>
              <li>WhatsApp + Direct Payment integration</li>
              <li>One-time setup fee — no monthly costs</li>
              <li>Update prices and content anytime</li>
            </ul>
            <a class="why-cta-btn" href="${wa}" target="_blank" rel="nofollow">Book a Free Consultation →</a>
          </div>
        </div>
      </div>
    </section>`;
...
<head>
  ...
  <title>${escapeHtml(property.name)} | Direct Booking Preview</title>
  <meta name="description" content="${escapeHtml(property.name)} — ${escapeHtml(property.location)}. Book direct and save on commissions.">
  <style>${CSS}</style>
</head>
<body>

  <div class="demo-bar">
    This page was custom-built for <strong>${escapeHtml(property.name)}</strong> &nbsp;·&nbsp;
    <a href="${wa}" target="_blank" rel="nofollow">Claim your site today →</a>
  </div>
  ...
  <nav class="nav" aria-label="Main Navigation">
    ...
    <div class="nav-links">
      ...
      <a href="#why">Why Direct?</a>
      <a href="#contact" class="nav-cta">Inquire Now</a>
    </div>
  </nav>

  <header class="hero" style="background-image:url('${cssUrl(heroImage)}')">
    <div class="hero-grid">
      <div>
      ...
      <aside class="booking-card" aria-label="Booking Inquiry">
        <div class="booking-card-head">
          <h2>Quick Inquiry</h2>
          <p>Contact directly — no platform commissions.</p>
        </div>
        <div class="booking-row">
          <div class="field"><span class="field-label">Check-in</span><strong>Select Dates</strong></div>
          <div class="field"><span class="field-label">Check-out</span><strong>Select Dates</strong></div>
        </div>
        <div class="booking-row">
          <div class="field"><span class="field-label">Guests</span><strong>Number of guests</strong></div>
          <div class="field"><span class="field-label">Stay Type</span><strong>${escapeHtml(property.type || "Stay")}</strong></div>
        </div>
        <a class="btn btn-moss" href="${wa}" target="_blank" rel="nofollow">Connect via WhatsApp →</a>
        <div class="commission-badge">
          <span class="commission-badge-icon">✓</span>
          <span>No Airbnb/Booking.com commission — keep 100% of your earnings.</span>
        </div>
      </aside>
    </div>
  </header>
      ...
      <div class="section-inner">
        <div class="section-head">
          <div>
            ...
            <h2>Everything your guests need to decide.</h2>
          </div>
          <p class="lead">Property details, stunning visuals, and direct booking all on one page. Guests find what they need and contact you directly.</p>
        </div>
        <div class="feature-grid">
          <div class="image-panel" style="background-image:url('${cssUrl(featureImage)}')" role="img" aria-label="${escapeHtml(property.name)} image"></div>
      ...
      <div class="section-inner">
        <div class="section-head">
          <div>
            ...
            <h2>Your site answers their questions upfront.</h2>
          </div>
          <p class="lead">Guests don't have to hunt for features on other platforms — everything is here, clear and fast.</p>
        </div>
      ...
      <div class="section-inner">
        <div class="section-head">
          <div>
            ...
            <h2>Photos drive the booking decision.</h2>
          </div>
          <p class="lead">A well-structured site with the right visuals delivers much higher conversion than generic platform listings.</p>
        </div>
        <div class="gallery" aria-label="${escapeHtml(property.name)} gallery">${renderGallery(gallery)}</div>
      </div>
    </section>
      ...
      <div class="section-inner">
        <div class="section-head">
          <div>
            ...
            <h2>Reach out directly for bookings.</h2>
          </div>
          <p class="lead">No middleman. Guests call you directly, and payments come straight to you.</p>
        </div>
        <div class="contact-grid">
          <div class="map-card" style="background-image:url('${cssUrl(gallery[0] || heroImage)}')">
            ...
            <p>Detailed location and area guide added upon launch.</p>
          </div>
          <div class="contact-card">
            <h2>Availability & Reservations</h2>
            <div class="contact-lines">
              <div class="contact-line">
                <span class="contact-line-label">Instagram</span>
                <span class="contact-line-val">@${escapeHtml(property.contact.instagram)}</span>
              </div>
              <div class="contact-line">
                <span class="contact-line-label">Address</span>
                <span class="contact-line-val">${escapeHtml(property.contact.address || property.location)}</span>
              </div>
            </div>
            <div class="contact-actions">
              <a class="btn btn-moss" href="${wa}" target="_blank" rel="nofollow">Message on WhatsApp</a>
            </div>
            <p class="contact-note">This draft site will be finalized with prices, availability, and policies upon business approval.</p>
          </div>
        </div>
      </div>
    </section>
  ...
  <footer>
    ...
    <span>This demo was prepared for outreach purposes · Contact to launch your professional site</span>
  </footer>
...
const usSlugs = [
  "cabin-collective-broken-bow",
  "broken-bow-cabin-life",
  "broken-bow-vacay",
  "broken-bow-cabin-getaways",
...
for (const slug of usSlugs) {
  const jsonPath = path.join(dataDir, `${slug}.json`);
  if (!fs.existsSync(jsonPath)) continue;
  
  const property = JSON.parse(fs.readFileSync(jsonPath, "utf8"));
...
// Create a list page for US demos
const usRows = usSlugs.map(slug => {
    const jsonPath = path.join(dataDir, `${slug}.json`);
    ...
    return `
      <a class="demo-item" href="${p.slug}.html">
        <div>
          ...
          <span>${escapeHtml(p.type || "Stay")} — ${escapeHtml(p.location)}</span>
        </div>
        <span class="arrow">→</span>
      </a>`;
}).join("");

fs.writeFileSync(path.join(outDir, "us-demos.html"), `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>US Hospitality Demo Showcase</title>
  <style>
...
<body>
  <main>
    <h1>Demo Showcase</h1>
    <p class="sub">Custom draft sites for boutique hotels and cabins. Stop paying commissions, start booking direct.</p>
    <div class="list">${usRows}</div>
  </main>
</body>
</html>`);

console.log("built demos/public/us-demos.html");
